"""Run Project 8 Problem 3(a) with the notebook default parameters."""
...
def main() -> None:
    ...
    print("Running GA with notebook default parameters...")
    ...
    for ax, obs, pred, clean, label, color in [
        (axes[0], data["h_obs"], h_pred_train, data["h_clean"], "Training Season", "steelblue"),
        (axes[1], data["h_obs_test"], h_pred_test, data["h_clean_test"], "Held-out Test Season", "darkorange"),
    ]:
    ...
    fig.suptitle(f"Default Noise Parameter Recovery - Train MSE: {train_mse:.4f} | Test MSE: {test_mse:.4f}", fontsize=11, y=1.01)
    fig.tight_layout()
    fig.savefig("figures/trajectories_3a.png", dpi=150, bbox_inches="tight")
