"""Run GA parameter recovery with increased noise for Project 8, Problem 3c."""
...
NOISE_SIGMA = 0.20   # increased from default 0.03
...
# ── 1. Generate high-noise data ───────────────────────────────────────────────
data = generate_dataset(noise_sigma=NOISE_SIGMA, seed=3)
...
h_pred_test = simulate_heights(
    ...
    time=time,
    W=data["W_test"], F=data["F_test"], S=data["S_test"],
    positions=positions,
)
...
# ── 4. Print parameter recovery table ────────────────────────────────────────
...
for ax, (h_obs, h_pred, h_clean, label, color) in zip(axes, [
    (data["h_obs"],      h_pred_train, data["h_clean"],      "Training Season (high noise)", "steelblue"),
    (data["h_obs_test"], h_pred_test,  data["h_clean_test"],  "Test Season (high noise)",     "darkorange"),
]):
...
plt.suptitle(
    f"High Noise ($\\sigma={NOISE_SIGMA}$) — Train MSE: {train_mse:.4f}  |  Test MSE: {test_mse:.4f}",
    fontsize=11, y=1.01,
)
plt.tight_layout()
plt.savefig("figures/trajectories_3c.png", dpi=150, bbox_inches="tight")
